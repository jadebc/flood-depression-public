...
# Create tables reported in the paper
#############################################
...
table1 <- tabdata %>%
  dplyr::select(mother_edu,
                ...
                flood_union,
                flood_compound,
                time_since_flood_compound, 
                inside_hh_flooded,
                num_days_home_flooded,
                latrine_flooded,
                num_days_latrine_flooded,
                tubewell_flooded,
                num_days_tubewell_flooded,
                flood_prepared) %>% 
  # Create variable for income less than 12,001  
  mutate(income_less_12 = ifelse(income < 6, 1, 0)) %>% 
  dplyr::select(-income) %>% 
  # Move income_less_12 after hhsize
  relocate(income_less_12, .after = floor_mud)  

# Create variables for wall material 
table1 <- table1 %>% 
  mutate(wall_jute_bamboo_straw = ifelse(wall_jute == 1 | wall_bamboo == 1 | wall_straw == 1, 1, 0),
         wall_brick_cement = ifelse(wall_brick == 1 | wall_cement == 1, 1, 0)) %>% 
  dplyr::select(-wall_jute, -wall_bamboo, -wall_straw, -wall_brick, -wall_cement)

# Create variables for basic, limited, unimproved sanitation and basic water
table1 <- table1 %>% 
  mutate(san_ladder = ifelse(san_ladder == "Don't know/Missing", NA, san_ladder),
         san_basic = ifelse(san_ladder == "Basic", 1, 0),
         san_limited = ifelse(san_ladder == "Limited", 1, 0),
         san_unimproved = ifelse(san_ladder == "Unimproved", 1, 0),
         basic_water = ifelse(water_ladder == "Basic", 1, 0)) %>% 
  dplyr::select(-san_ladder, -water_ladder)
...
perc_vars <- c("roof_bamboo",
               "roof_tin", 
               "wall_jute_bamboo_staw",
               "wall_brick_cement",
               "wall_tin", 
               "floor_mud",
               "san_basic",
...
labels <- c(
  ...
  "Roof material is bamboo",
  "Roof material is tin", 
  "Wall material is jute, bamboo or straw",
  "Wall material is brick or cement",
  "Wall material is tin", 
  "Floor material is mud",
  "Sanitation is basic",
  "Sanitation is limited",
  "Sanitation is unimproved",
  "Open defecation of children <3 years",
  "Handwashing station with soap and water",
  "Basic water",
  "Monthly income < 12,001 taka",
  "Has electricity",
  "Owns mobile",
  "Owns livestock",
  "Father works in agriculture",
  "Union flooded for at least one day in past 6 months",
  "Compound flooded for at least one day in past 6 months", 
  "Number of months ago that the compound flooded", 
  "Inside of the home flooded in the past 6 months",
  "Number of days the home was flooded",
  "Latrine flooded in past 6 months", 
  "Number of days latrine was flooded",
  "Tubewell flooded in past 6 months",
  "Number of days the tubewell was flooded",
  "Respondent feels prepared to handle a flood if it happened tomorrow"
)
